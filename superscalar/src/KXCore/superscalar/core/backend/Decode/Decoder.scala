...
object FUTypeControlField extends DecodeField[Instruction, UInt] {
  ...
  def genTable(op: Instruction): BitPat = {
    op match {
      ...
      case CPUCFG                                                              => BitPat(FUType.FUT_CSR.asUInt)
...
object IMMTypeControlField extends DecodeField[Instruction, UInt] {
  ...
  def genTable(op: Instruction): BitPat = {
    op match {
      ...
      case CACOP                                                         => BitPat(IMMType.IMM_12.asUInt)
      ...
      case BREAK | SYSCALL | DBAR | IBAR | INVTLB | IDLE                 => BitPat(IMMType.IMM_15U.asUInt)
...
object EXUOPControlField extends DecodeField[Instruction, UInt] {
  ...
  def genTable(op: Instruction): BitPat = {
    op match {
      ...
      case LD_B                                                                => BitPat(EXUType.EXU_LDB.asUInt)
      case LD_H                                                                => BitPat(EXUType.EXU_LDH.asUInt)
      case LD_W                                                                => BitPat(EXUType.EXU_LDW.asUInt)
      case ST_B                                                                => BitPat(EXUType.EXU_STB.asUInt)
      case ST_H                                                                => BitPat(EXUType.EXU_STH.asUInt)
      ...
      case CPUCFG                                                              => BitPat(EXUType.EXU_CPUCFG.asUInt)
      case RDCNTID_W_0 | RDCNTID_W_1 | RDCNTID_W_2 | RDCNTID_W_3 | RDCNTID_W_4 => BitPat(EXUType.EXU_RDCNTID.asUInt)
      case RDCNTVL_W                                                           => BitPat(EXUType.EXU_RDCNTVL.asUInt)
      case RDCNTVH_W                                                           => BitPat(EXUType.EXU_RDCNTVH.asUInt)
      case TLBSRCH                                                             => BitPat(EXUType.EXU_TLBSRCH.asUInt)
      case TLBRD                                                               => BitPat(EXUType.EXU_TLBRD.asUInt)
      case TLBWR                                                               => BitPat(EXUType.EXU_TLBWR.asUInt)
      case TLBFILL                                                             => BitPat(EXUType.EXU_TLBFILL.asUInt)
      case INVTLB                                                              => BitPat(EXUType.EXU_INVTLB.asUInt)
      case CACOP                                                               => BitPat(EXUType.EXU_CACOP.asUInt)
      case ERTN                                                                => BitPat(EXUType.EXU_ERTN.asUInt)
      case IDLE                                                                => BitPat(EXUType.EXU_IDLE.asUInt)
...
object RS1ControlField extends DecodeField[Instruction, UInt] {
  ...
  def genTable(op: Instruction): BitPat = {
    op match {
      ...
      case BREAK | SYSCALL                                                     => BitPat(RS1From.rs1None.asUInt)
      case DBAR | IBAR                                                         => BitPat(RS1From.rs1None.asUInt)
      case ERTN | IDLE                                                         => BitPat(RS1From.rs1None.asUInt)
      ...
      case TLBSRCH | TLBRD | TLBWR | TLBFILL                                   => BitPat(RS1From.rs1None.asUInt)
...
object RS2ControlField extends DecodeField[Instruction, UInt] {
  ...
  def genTable(op: Instruction): BitPat = {
    op match {
      ...
      case BREAK | SYSCALL                                                     => BitPat(RS2From.rs2None.asUInt)
      case LD_B | LD_H | LD_W | LD_BU | LD_HU                                  => BitPat(RS2From.rs2None.asUInt)
      case LL_W | DBAR | IBAR                                                  => BitPat(RS2From.rs2None.asUInt)
      case CPUCFG                                                              => BitPat(RS2From.rs2None.asUInt)
      ...
      case TLBSRCH | TLBRD | TLBWR | TLBFILL                                   => BitPat(RS2From.rs2None.asUInt)
      case ERTN | IDLE                                                         => BitPat(RS2From.rs2None.asUInt)
      case CACOP                                                               => BitPat(RS2From.rs2None.asUInt)
      ...
      case ST_B | ST_H | ST_W | SC_W                                           => BitPat(RS2From.rs2FromRd.asUInt)
...
object UniqControlField extends DecodeField[Instruction, Bool] {
  ...
  def genTable(op: Instruction): BitPat = {
    op match {
      ...
      case LL_W | SC_W | DBAR | IBAR                             => BitPat.Y(1)
...
class Decoder(implicit params: CoreParameters) extends Module {
  ...
  for (i <- 0 until coreWidth) {
    val ine = !possiblePatterns.map(_.inst === io.req(i).inst).reduce(_ || _) ||
      (io.req(i).inst === INVTLB.inst && io.req(i).inst(4, 0) > 6.U)
