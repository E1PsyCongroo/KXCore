...
class CSRUnit(implicit params: CoreParameters) extends Module {
  ...
  val era       = RegInit(0.U.asTypeOf(UInt(32.W)))
  val badv      = RegInit(0.U.asTypeOf(UInt(32.W)))
  val eentry    = RegInit(0.U.asTypeOf(new EENTRY))
  val tlbidx    = RegInit(0.U.asTypeOf(new TLBIDX(log2Ceil(params.tlbNum))))
  val tlbehi    = RegInit(0.U.asTypeOf(new TLBEHI))
  val tlbelo0   = RegInit(0.U.asTypeOf(new TLBELO(params.commonParams.paddrWidth)))
  val tlbelo1   = RegInit(0.U.asTypeOf(new TLBELO(params.commonParams.paddrWidth)))
